...
Workers 에서도 `strict` 모드(기본)가 `dns.resolve4`/`resolve6` 기반으로
동작하지만, DNS subrequest 비용을 아끼고 싶으면 환경변수
`OGPEEK_SSRF_MODE=hostname` 으로 전환할 수 있다 (Workers Vars 또는
`wrangler.json` `vars`). hostname 문자열 검사만 하며 DNS 를 타지 않는다.
...
| `OGPEEK_SSRF_MODE` | `strict` | `strict`(`resolve4/6` 기반 DNS 리졸브 + 사설 IP 차단, Node·Workers 공용) / `hostname`(문자열 검사만, DNS subrequest 없음) / `off`(검사 비활성) |
