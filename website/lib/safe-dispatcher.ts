...
// 에 뒤바뀔 수 있다. 소켓 연결 직전 한 번 더 풀고 첫 공인 IP 만 골라 connect
// 한다. 전부 사설이면 FetchError 로 차단.
//
// lookup 시그니처는 node:dns lookup 과 동일: (hostname, opts, callback).
// `all: true` 로 모든 레코드를 받아서 IPv4 공인 + IPv6 사설 같은 섞인 응답으로
// 우회하는 시나리오를 차단한다. `verbatim: true` 로 OS 정렬 재정렬 회피
// (pre-flight 와 동일 기준).
const safeLookup: LookupFunction = (hostname, _opts, callback) => {
