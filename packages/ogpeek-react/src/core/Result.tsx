...
// Server-friendly composite: no Context, no Provider, no client hooks.
// Resolves dict once and drills `lang` / `dict` / `composed=true` to every
// child explicitly. The wrapping div is the only `.ogpeek-root` in the
// subtree — children skip their own root class so inline token overrides
// on the wrapper still cascade. See `src/Result.tsx` for the client
// counterpart that uses Context to avoid the prop drilling.
