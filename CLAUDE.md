...
3. **SSRF 가드는 on-by-default**. `fetchHtml`은 옵션 없이 호출하면 `"strict"`
   모드 — `node:dns/promises` 의 `resolve4`/`resolve6` 로 A·AAAA 레코드를
   직접 질의해 사설/루프백/링크로컬 대역 차단 — 로 동작한다. Node.js 와
   `nodejs_compat` 플래그가 켜진 Cloudflare Workers 양쪽에서 같은 코드로
   굴러간다. DNS subrequest 비용을 아끼거나 `node:dns/promises` 자체가 없는
   런타임(예: Vercel Edge)에서는 `{ ssrf: "hostname" }` 또는 환경변수
   `OGPEEK_SSRF_MODE=hostname` 으로 문자열-only 검사로 전환할 수 있다. 이
   기본값을 뒤집지 마라.
   리디렉션은 반드시 `redirect: "manual"`로 받아서 hop마다 선택된 가드를
   다시 돌린다. 레거시 `OGPEEK_ALLOW_PRIVATE_NETWORK=1` / `allowPrivateNetwork`
   옵션은 호환을 위해 유지하지만 `ssrf` 옵션이 우선한다.
...
Workers 에서도 `strict` 모드(기본)가 `resolve4`/`resolve6` 기반으로 동작하지만,
DNS subrequest 비용을 아끼고 싶으면 `OGPEEK_SSRF_MODE=hostname` 으로 전환할
수 있다 (Workers Vars 또는 `wrangler.json` `vars`). 이 모드는 hostname 문자열
검사만 하며 DNS 를 전혀 타지 않는다.
